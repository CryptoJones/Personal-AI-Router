...
may use a key at all. The file and inline keys are separate sources: a key
file that is refused contributes no keys, but inline keys still work.

On Kubernetes, supply the key through `NVPAIR_PROXY_API_KEYS` from a Secret
with `valueFrom.secretKeyRef`. A Secret mounted as a file and pointed to by
`NVPAIR_PROXY_API_KEYS_FILE` also works under a pod `fsGroup`, which mounts it
root-owned and group-readable (`0440`): the proxy accepts group read on a
root-owned file whose group it belongs to. Set the allowlist to the pod
network's CIDR: on OVN-Kubernetes, a pod reaching a `hostNetwork` node on the same
host arrives from its pod IP, not the node's address.
